...
#ifndef _aer_monte_carlo_state_hpp
#define _aer_monte_carlo_state_hpp
...
#include "monte_carlo_thrust.hpp"
#endif
...
//=========================================================================
// MonteCarlo State subclass
//=========================================================================

template <class densmat_t = QV::MonteCarlo<double>>
...
public:
  ...
  // Initialize OpenMP settings for the underlying MonteCarlo class
  void initialize_omp();
  ...
  cmatrix_t reduced_monte_carlo(const reg_t &qubits, bool last_op = false);
...
protected:
  ...
  void apply_save_monte_carlo(const Operations::Op &op,
  ...
  // Return the reduced density matrix for the simulator
  cmatrix_t reduced_monte_carlo_helper(const reg_t &qubits,
  ...
  // NOTE: This is twice the number of qubits in the MonteCarlo since it
...
void State<densmat_t>::initialize_qreg(uint_t num_qubits, densmat_t &&state) {
  if (state.num_qubits() != num_qubits) {
    ...
    throw std::invalid_argument("MonteCarlo::State::initialize_qreg: "
...
void State<densmat_t>::apply_op(const Operations::Op &op,
  ...
  if (BaseState::creg().check_conditional(op)) {
    ...
    case OpType::save_densmat:
      apply_save_monte_carlo(op, result, final_ops);
      break;
    ...
    default:
      throw std::invalid_argument(
          "MonteCarlo::State::invalid instruction \'" + op.name + "\'.");
    }
  }
}
...
template <class densmat_t>
void State<densmat_t>::apply_save_monte_carlo(const Operations::Op &op,
                                                 ExperimentResult &result,
                                                 bool last_op) {
  result.save_data_average(BaseState::creg(), op.string_params[0],
                           reduced_monte_carlo(op.qubits, last_op), op.type,
                           op.save_type);
}
...
void State<densmat_t>::apply_save_state(const Operations::Op &op,
  ...
  // Default key
  std::string key = (op.string_params[0] == "_method_") ? "monte_carlo"
                                                        : op.string_params[0];
  if (last_op) {
...
template <class densmat_t>
cmatrix_t State<densmat_t>::reduced_monte_carlo(const reg_t &qubits,
                                                   bool last_op) {
  ...
  if (qubits.empty()) {
    ...
    if ((qubits.size() == BaseState::qreg_.num_qubits()) &&
      ...
      reduced_state = reduced_monte_carlo_helper(qubits, qubits_sorted);
    }
  }
...
template <class densmat_t>
cmatrix_t
State<densmat_t>::reduced_monte_carlo_helper(const reg_t &qubits,
                                                const reg_t &qubits_sorted) {
...
void State<densmat_t>::apply_gate(const Operations::Op &op) {
  ...
  if (it == gateset_.end())
    throw std::invalid_argument(
        "MonteCarloState::invalid gate instruction \'" + op.name + "\'.");
  switch (it->second) {
  case Gates::u3:
  ...
  default:
    ...
    throw std::invalid_argument(
        "MonteCarlo::State::invalid gate instruction \'" + op.name + "\'.");
  }
}
