...
#ifndef _montecarlo_executor_hpp_
#define _montecarlo_executor_hpp_
...
protected:
  ...
  void apply_save_monte_carlo(CircuitExecutor::Branch &root,
                                 const Operations::Op &op, ResultItr result,
                                 bool final_op);
...
#pragma omp parallel for
    ...
    throw std::runtime_error(
        "MonteCarloChunk::initialize input vector is incorrect length. "
        "Expected: " +
...
bool Executor<state_t>::apply_branching_op(CircuitExecutor::Branch &root,
  ...
  if (Base::states_[root.state_index()].creg().check_conditional(op)) {
    ...
    case Operations::OpType::save_state:
      ...
      break;
    case Operations::OpType::save_densmat:
      apply_save_monte_carlo(root, op, result, final_op);
      break;
    case Operations::OpType::save_probs:
    case Operations::OpType::save_probs_ket:
...
template <class densmat_t>
void Executor<densmat_t>::apply_save_monte_carlo(const Operations::Op &op,
                                                    ExperimentResult &result,
                                                    bool last_op) {
  result.save_data_average(Base::states_[0].creg(), op.string_params[0],
...
template <class densmat_t>
cmatrix_t Executor<densmat_t>::reduced_monte_carlo(const reg_t &qubits,
                                                      bool last_op) {
...
template <class densmat_t>
cmatrix_t
Executor<densmat_t>::reduced_monte_carlo_helper(const reg_t &qubits,
                                                   const reg_t &qubits_sorted) {
  uint_t iChunk;
...
template <class densmat_t>
void Executor<densmat_t>::apply_save_monte_carlo(
    CircuitExecutor::Branch &root, const Operations::Op &op, ResultItr result,
    bool final_op) {
  cmatrix_t mat;
  mat = Base::states_[root.state_index()].reduced_monte_carlo(op.qubits,
                                                                 final_op);
