...
#ifndef _qv_monte_carlo_hpp_
#define _qv_monte_carlo_hpp_
...
class MonteCarlo : public UnitaryMatrix<data_t> {
...
public:
  ...
  explicit MonteCarlo(size_t num_qubits);
  MonteCarlo(const MonteCarlo &obj){};
  MonteCarlo &operator=(const MonteCarlo &obj) = delete;
  MonteCarlo &operator=(MonteCarlo &&obj) noexcept;

  //-----------------------------------------------------------------------
  // Utility functions
  //-----------------------------------------------------------------------

  // Return the string name of the MonteCarlo class
  static std::string name() { return "monte_carlo"; }

  // Initializes the current vector so that all qubits are in the |0> state.
  void initialize();

  // initialize from existing state (copy)
  void initialize(const MonteCarlo<data_t> &obj) {
...
MonteCarlo<data_t>::MonteCarlo(size_t num_qubits)
    : UnitaryMatrix<data_t>(num_qubits) {}

template <typename data_t>
MonteCarlo<data_t> &
MonteCarlo<data_t>::operator=(MonteCarlo<data_t> &&obj) noexcept {
...
template <typename data_t>
void MonteCarlo<data_t>::initialize() {
...
template <typename data_t>
template <typename list_t>
void MonteCarlo<data_t>::initialize_from_vector(const list_t &vec) {
  if (BaseVector::data_size_ == vec.size()) {
    ...
    throw std::runtime_error("MonteCarlo::initialize input vector is "
...
template <typename data_t>
template <typename list_t>
void MonteCarlo<data_t>::initialize_from_vector(list_t &&vec) {
  if (BaseVector::data_size_ == vec.size()) {
    // Use base class initialize for already vectorized matrix
...
template <typename data_t>
void MonteCarlo<data_t>::transpose() {
...
template <typename data_t>
reg_t MonteCarlo<data_t>::superop_qubits(const reg_t &qubits) const {
...
template <typename data_t>
cvector_t<double>
MonteCarlo<data_t>::vmat2vsuperop(const cvector_t<double> &vmat) const {
...
template <typename data_t>
void MonteCarlo<data_t>::apply_superop_matrix(const reg_t &qubits,
                                                 const cvector_t<double> &mat) {
  BaseVector::apply_matrix(superop_qubits(qubits), mat);
}

template <typename data_t>
void MonteCarlo<data_t>::apply_diagonal_superop_matrix(
    const reg_t &qubits, const cvector_t<double> &diag) {
  BaseVector::apply_diagonal_matrix(superop_qubits(qubits), diag);
}

template <typename data_t>
void MonteCarlo<data_t>::apply_unitary_matrix(const reg_t &qubits,
                                                 const cvector_t<double> &mat) {
...
template <typename data_t>
void MonteCarlo<data_t>::apply_diagonal_unitary_matrix(
    const reg_t &qubits, const cvector_t<double> &diag) {
  // Apply as single 2N-qubit matrix mult.
...
template <typename data_t>
void MonteCarlo<data_t>::apply_cnot(const uint_t qctrl, const uint_t qtrgt) {
...
template <typename data_t>
void MonteCarlo<data_t>::apply_cy(const uint_t qctrl, const uint_t qtrgt) {
  const reg_t qubits = {{qctrl, qtrgt}};
  apply_unitary_matrix(qubits, Linalg::VMatrix::CY);
}

template <typename data_t>
void MonteCarlo<data_t>::apply_phase(const uint_t q,
...
template <typename data_t>
void MonteCarlo<data_t>::apply_cphase(const uint_t q0, const uint_t q1,
                                         const complex_t &phase) {
  const complex_t iphase = std::conj(phase);
  // Lambda function for CZ gate
...
template <typename data_t>
void MonteCarlo<data_t>::apply_swap(const uint_t q0, const uint_t q1) {
  std::vector<std::pair<uint_t, uint_t>> pairs = {
...
template <typename data_t>
void MonteCarlo<data_t>::apply_x(const uint_t qubit) {
...
template <typename data_t>
void MonteCarlo<data_t>::apply_y(const uint_t qubit) {
...
template <typename data_t>
void MonteCarlo<data_t>::apply_toffoli(const uint_t qctrl0,
                                          const uint_t qctrl1,
                                          const uint_t qtrgt) {
...
template <typename data_t>
double
MonteCarlo<data_t>::expval_pauli(const reg_t &qubits,
...
template <typename data_t>
double MonteCarlo<data_t>::expval_pauli_non_diagonal_chunk(
...
template <typename data_t>
double MonteCarlo<data_t>::probability(const uint_t outcome) const {
  const auto shift = BaseMatrix::num_rows() + 1;
  return std::real(BaseVector::data_[outcome * shift]);
}

template <typename data_t>
void MonteCarlo<data_t>::apply_reset(const reg_t &qubits) {
...
inline std::ostream &operator<<(std::ostream &out,
                                const AER::QV::MonteCarlo<data_t> &m) {
